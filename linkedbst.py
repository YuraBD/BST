...
class LinkedBST(AbstractCollection):
    ...
    def range_find(self, low, high):
        ...
        def check_range(node):
            if node is None:
                ...
                check_range(node.left)
            elif node.data < low:
                check_range(node.right)
            else:
                elements.add(node.data)
                check_range(node.left)
                check_range(node.right)

        check_range(self._root)
    ...
    def rebalance(self):
        ...
        def add_el(el_list):
            if not el_list:
                return
            pos = len(el_list)//2
            self.add(el_list[pos])
            add_el(el_list[0:pos])
            add_el(el_list[pos+1:len(el_list)])
    ...
    def successor(self, item):
        ...
        while curr is not None:
            #if item < curr.data and (succ is None or curr.data < succ):
            if item < curr.data:
                succ = curr.data
                curr = curr.left
            else:
    ...
    def demo_bst(self, path):
        ...
        find_words = random.sample(words, 10000)

        t_start = time.perf_counter()
        for word in find_words:
            words.index(word)
        t_stop = time.perf_counter()
        print("List: ", t_stop - t_start)

        bin_tree = LinkedBST(words[:900])
        t_start = time.perf_counter()
        for word in find_words:
            bin_tree.find(word)
        t_stop = time.perf_counter()
        print("Sorted binary tree: ", t_stop - t_start)
        ...
        for word in find_words:
            bin_tree.find(word)
        t_stop = time.perf_counter()
        print("Random binary tree: ", t_stop - t_start)
        ...
        for word in find_words:
            bin_tree.find(word)
        t_stop = time.perf_counter()
        print("Balanced  binary tree: ", t_stop - t_start)
